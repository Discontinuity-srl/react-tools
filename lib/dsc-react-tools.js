...
export default {
  ...
  closeModalPannel() {
    this.dscReactToolsView.setSuggestion("")
  ...
  renameAction() {
    ...
    let fromName = path.basename(filePath, ".js")
    if (fromName === "index") {
      fromName = path.basename(path.dirname(filePath))
    }
    ...
    this.dscReactToolsView.setPromptMessage(
      `Enter the new name for ${fromName}`
    )
    this.dscReactToolsView.setSuggestion(fromName)
    this.dscReactToolsView.onCreate = name => onCreate(name)
    this.dscReactToolsView.close = () => this.closeModalPannel()
